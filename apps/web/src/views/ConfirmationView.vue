...
function startPolling(): void {
  ...
  pollTimer = setInterval(() => {
    void (async () => {
      ...
      try {
        const fresh = await api.getBooking(id.value);
        booking.value = fresh;
  ...
  <template v-else-if="booking">
    <div v-if="booking.status === 'PENDING'" class="bx-empty">
      ...
      <p class="bx-empty-copy">
        Your Xendit payment is being confirmed. Tickets appear here automatically once the payment
        callback arrives — no need to pay again.
      </p>
